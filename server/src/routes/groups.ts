...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const groups = await query<mysql.RowDataPacket[]>('SELECT * FROM groups_table ORDER BY name');

    const result = await Promise.all(
      groups.map(async g => {
        const teams = await query<mysql.RowDataPacket[]>(
          'SELECT * FROM teams WHERE group_name = ? ORDER BY id',
          [g.name]
        );
        const standings = await query<mysql.RowDataPacket[]>(
